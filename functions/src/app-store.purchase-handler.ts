...
export class AppStorePurchaseHandler extends PurchaseHandler {
  ...
  private async handleValidation(userId: string, token: string) : Promise<boolean> {
    ...
    try {
      products =
       await appleReceiptVerify.validate({receipt: token});
    } catch (e) {
    ...
    for (const product of products) {
      ...
      case "SUBSCRIPTION":
        ...
        await this.firebaseCalls.createOrUpdatePurchase({
          ...
          userId: userId,
          // eslint-disable-next-line max-len
          purchaseDate: firestoreAdmin.firestore.Timestamp.fromMillis(product.purchaseDate),
          type: productData.type,
          // eslint-disable-next-line max-len
          expiryDate: firestoreAdmin.firestore.Timestamp.fromMillis(product.expirationDate ?? 0),
  ...
  handleServerEvent1 = functions.https.onRequest(async (req, res) => {
    ...
    const eventData = {
      ...
      purchaseDate: signedInfo.purchaseDate,
